...
int main(int argc, char **argv) {
  ...
  if (gemcOpt.optMap["RANDOM"].args == "TIME") {
    gemc_splash.message(" Initializing CLHEP Random Engine from local time " + stringify((double)time(NULL)) +
                        ", cpu clock " + stringify((double)clock()) + " and process id " + stringify(getpid()) + ".");
  ...
  // if (use_gui) G4UIsession *session = new G4UIQt(1, argv);
  ...
  for (it = ExpHall->SeDe_Map.begin(); it != ExpHall->SeDe_Map.end(); it++) it->second->hitProcessMap = &hitProcessMap;

  gemc_splash.message(" Executing initial directives...\n");
  vector<string> init_commands = init_dmesg(gemcOpt);
  for (unsigned int i = 0; i < init_commands.size(); i++) UImanager->ApplyCommand(init_commands[i].c_str());
